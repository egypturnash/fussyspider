...
class TaskEditViewController: UIViewController {
    ...
    if sender!.tag == 1 {
      ...
      if let hidden = dateFormatter.dateFromString(reminderHidden.text!) {
        if let calendar = NSCalendar(calendarIdentifier: NSCalendarIdentifierGregorian) {
          let components = calendar.components([.Day, .Month, .Year, .Hour, .Minute], fromDate: hidden)
          reminder.startDateComponents = components
        }
      }
      taskStore.addTask(FSTask(task: reminder))
    }
  }
  ...
  override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
    ...
    reminderDate.endEditing(true)
    reminderHidden.endEditing(true)
  }
}
